...
export function HeroSyncSimulator() {
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#0B0A14] rounded-xl md:rounded-2xl overflow-hidden border border-white/10 flex flex-col shadow-2xl select-none text-left">
        {/* macOS App Titlebar */}
        <div className="bg-[#0e0c1a] px-3.5 md:px-4 py-2.5 border-b border-white/5 flex items-center">
        ...
        <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] min-h-[380px] sm:min-h-[440px] md:min-h-[500px] overflow-hidden bg-black flex flex-col justify-between">
          ...
          {showChat && (
            <div className="absolute top-16 sm:top-18 md:top-20 right-3 sm:right-4 md:right-5 w-64 sm:w-72 md:w-80 z-25 backdrop-blur-2xl bg-[#141022]/95 border border-purple-400/30 rounded-2xl p-3.5 shadow-2xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-200 text-left">
              ...
              {/* Chat Message List */}
              <div className="space-y-2 max-h-44 overflow-y-auto text-xs pr-1 text-left">
                {chatMessages.map((msg) => (
                  <div
                    key={msg.id}
                    className="p-2 rounded-lg bg-white/5 border border-white/5 text-gray-200 leading-snug text-left"
                  >
                ...
                <div className="p-1.5 rounded-lg bg-purple-500/10 border border-purple-500/20 text-[11px] text-purple-200 flex items-center gap-1.5 text-left">
              ...
              <form onSubmit={handleSendMessage} className="flex gap-1.5 pt-1">
                <input
                  ...
                  className="flex-1 h-8 bg-black/40 border border-white/10 rounded-lg px-2.5 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-purple-400 text-left"
                />
                <button
                  type="submit"
                  aria-label="Send message"
                  className="w-8 h-8 shrink-0 aspect-square rounded-lg bg-purple-600 hover:bg-purple-500 text-white flex items-center justify-center transition-colors cursor-pointer"
                >
